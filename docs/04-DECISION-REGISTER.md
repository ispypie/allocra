...
| DEC-021 | Buffers deferred from slice | The first slice reserved exact `[start, start+duration)` windows; setup/cleanup buffers deferred to keep the slice thin. **Superseded by DEC-029** (buffers now implemented) | SUPERSEDED | 2026-08-01 | PRD-SCH-006, OQ-BUF-1 | ADR-006 |
...
| DEC-029 | Lead/setup/cleanup buffers | The appointment window stays client-facing; each resource is reserved for `[start − lead − setup, end + cleanup)`. The engine checks reservations against the buffered window (availability against the appointment window); reservations are stored buffered so the exclusion constraint protects turnover. Supersedes DEC-021 | ACCEPTED | 2026-08-02 | PRD-SVC-009, PRD-RES-012, PRD-SCH-006 | ADR-004, ADR-006 |
...
- 2026-08-02 — Added DEC-029 (lead/setup/cleanup buffers); superseded DEC-021 (buffers no longer deferred).
