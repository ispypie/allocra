...
| OQ-TIME-1 | Time zone handling for bookings across locations? | **Partially resolved (PRD-AVL-007):** each `Location` has an IANA timezone; operating hours, closures and located-resource availability are interpreted in it (DST-aware). Booking instants are stored/compared in UTC (`timestamptz`, `tstzrange`). Remaining: client display formatting and per-request timezone conventions. | ASSUMED | Medium. |
